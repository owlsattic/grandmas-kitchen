...
// Admin listing with optional ?approved=true|false and ?q=searchterm
// Always dedupes by product_num (case-insensitive), newest wins.
...
export const onRequestGet = async ({ request, env }) => {
  try {
    ...
    if (q) {
      const term = `*${q}*`;
      // (my_title ILIKE or amazon_title ILIKE)
      sb.searchParams.set(
        "or",
    ...
    // We sorted newest first, so the first occurrence wins.
    const seen = new Set();
    const unique = [];
    for (const row of rows || []) {
      const key = (row.product_num || "").toLowerCase();
      if (!key) {
        unique.push(row); // keep if no key (rare / transitional)
        continue;
      }
